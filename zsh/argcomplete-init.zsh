...
if [ -x "$pip_show_argcomplete" ]; then
    ...
    this_dir=${0:a:h}
    typeset -a lines=("${(f)"$(<$this_dir/../config/argcomplete/compat_programs.txt)"}")
    unset this_dir
    ...
    argcomplete_registration_script="$(python3 "$argcomplete_registration_broker" --shell zsh "$target_marker")"
