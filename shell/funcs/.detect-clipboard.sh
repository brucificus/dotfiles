...
# Taken and modified from https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/fe4b5659863c388786986d70fa6d1bb66b00afb6/lib/clipboard.zsh.
...
function detect-clipboard() {
  if [[ "${OSTYPE}" == darwin* ]] && command_exists pbcopy && command_exists pbpaste; then
    function clipcopy() { cat "${1:-/dev/stdin}" | pbcopy; }
    function clippaste() { pbpaste; }
  elif [[ "${OSTYPE}" = @(cygwin|msys)* ]]; then
    function clipcopy() { cat "${1:-/dev/stdin}" > /dev/clipboard; }
    function clippaste() { cat /dev/clipboard; }
  elif command_exists clip.exe && command_exists powershell.exe; then
    function clipcopy() { cat "${1:-/dev/stdin}" | clip.exe; }
    function clippaste() { powershell.exe -noprofile -command Get-Clipboard; }
  elif [ -n "${WAYLAND_DISPLAY:-}" ] && command_exists wl-copy && command_exists wl-paste; then
  ...
  elif command_exists doitclient; then
    function clipcopy() { cat "${1:-/dev/stdin}" | doitclient wclip; }
    function clippaste() { doitclient wclip -r; }
  elif command_exists win32yank; then
    function clipcopy() { cat "${1:-/dev/stdin}" | win32yank -i; }
    function clippaste() { win32yank -o; }
  elif [[ $OSTYPE == linux-android* ]] && command_exists termux-clipboard-set; then
    function clipcopy() { cat "${1:-/dev/stdin}" | termux-clipboard-set; }
    function clippaste() { termux-clipboard-get; }
  elif [ -n "${TMUX:-}" ] && command_exists tmux; then
    function clipcopy() { tmux load-buffer "${1:--}"; }
    function clippaste() { tmux save-buffer -; }
  else
    append_profile_suggestions "# TODO: 🔨 Install clipboard support for current platform. (e.g. via xclip, xsel, lemonade, doitclient, win32yank, etc.)"
